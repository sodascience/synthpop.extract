...
synp_get_param <- function(df, synds) {
  ...
  # extract betas and intercepts per level when "polyr"
  for (i in 2:length(params)) {
    ...
    if(used_methods[[i]]=="polr"){
      # extract intercept
      intercepts <- as.data.frame(params[[i]]$zeta)
      # specify the column name
      colnames(intercepts) <- "Intercept" 
      parameters <- as.data.frame(coef(params[[i]]))
      # extract beta values
      betas <- parameters[!rownames(parameters) %in% rownames(intercepts), 1, drop=F]
      ...
      par_list[[i]] <- data.frame(
        varname = c(paste("Intercept", rownames(intercepts)), rownames(betas)),
        param = c(paste0("b0","_", rownames(intercepts)) , paste0("b", 1:nrow(betas))),
        value = c(intercepts$Intercept, betas$Value)
      )
    }
  }
...
synp_gen_syndat <- function(par_list, n = 1000) {
  ...
  for (i in 2:length(par_list)) {
    ...
    if (methods[i] == "polr"){
      # extract intercepts (cut-offs)
      intercepts <- cur_df[stringr::str_detect(cur_df[,2], "b0"), 3]
      # extract betas 
      betas <- cur_df[!stringr::str_detect(cur_df[,2], "b0"), 3]
      # linear predictor
      linearpred <- xp[,-1] %*% as.matrix(betas) # exclude intercept
      # compute logits for each cut-off
      logits <- matrix(NA, nrow = n, ncol = length(intercepts)) # storage for logits
      for(m in 1:length(intercepts)){
        logits[,m] <- intercepts[m] - linearpred 
      }
      odds <- exp(logits)
      ...
      # add (cumulative) prob  for the last category
      probs <- cbind(prob, 1)

      # extract the category names
      intercept_name <- stringr::str_split(stringr::str_remove(cur_df[stringr::str_detect(
        cur_df[,2], "b0"), 2], "b0_"), pattern="\\|", simplify=T)
      # store the category names in order
      colnames(probs) <- c(intercept_name[,1], intercept_name[length(intercepts), 2])
      # create synthetic data
      syndat[,col_nm[i]] <- factor(colnames(probs)[idx] , levels=colnames(probs))
    }
