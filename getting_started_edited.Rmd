...
1.  Upload a code file we have prepared (`sythpop_extractor.R`) to the CBS RA environment.
2.  Estimate a parametric generative model for the dataset using the well-known `synthpop` 
    package in R.
3.  Use our code to extract parameters of this generative model and ensure disclosure control according to 
...
3.  Export the disclosure controlled parameters to an `xlsx` file.
4.  Offer the `xlsx` file to the microdata team for output control.
5.  Outside the CBS RA environment, download the `xlsx` file and load the parameters back in R.
6.  Generate synthetic data based on the model parameters.
7.  Run the original analysis on the synthetic data and report differences.
